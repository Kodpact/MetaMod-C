...
static CallGameFunction()
{
	...
	for (Index = 0; Index < PluginCount; Index++)
	{
		...
		switch (MGlobals.Result)
		{
			...
			case MRES_SUPERCEDE: __asm MOV Result, EAX; return;
		}
	}
...
__declspec(naked) pfnServerDeactivate() { Dest = DEST_DLLFUNC; Ofs = offsetof(DLL_FUNCTIONS, pfnServerDeactivate); Count = 0; __asm { jmp CallGameFunction }; }
...
extern int MetaMod_Init(char *GameDir, MetaGlobals *MGlobalsPtr);
...
int Initialize(void)
{
	...
	MGlobals.OrigReturn = &Result;
	MGlobals.OverrideReturn = &Result;
	...
	if (MetaMod_Init(GameDir, &MGlobals))
		return -1;
